...
namespace Authority.ServiceImpl
{
	...
	public class ChatSessionService : IChatSessionService
	{
		...
		/// <returns></returns>
		public async Task SaveMessageAsync(string classRoomId, RedisMessage redisMessage)
		{
			await Task.Run(() => { SortedSetAdd($"{CommonConstant.CHAT_COMMON_PREFIX}{classRoomId}", redisMessage); });
		...
		/// <returns></returns>
		private bool SortedSetAdd<T>(string key, T value, double? score = null)
		{

			double scoreNum = score ?? _GetScore(key);
			return _redis.SortedSetAdd(key, ConvertJson<T>(value), scoreNum);
		}
